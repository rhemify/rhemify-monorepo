...
- **Signing model — `ows` only.** The demo signs with the agent's raw private key (the "ows" path resolver instrument — Own Wallet Signing). Pre-signing policy enforcement (6 rules: `daily_limit`, `max_per_transaction`, `domain_allowlist`, `standard_allowlist`, `vendor_blocked`, `approval_threshold`) gates every payment client-side BEFORE the key is asked for a signature — that's the audit-grade defense. But the agent still HOLDS the key. For production fleet treasuries you want a delegated-signing instrument (`squads` multisig / `ika` 2PC-MPC / `privy` passkey) — the path resolver has those instruments registered as stubs but their executors are not yet implemented. Live e2e via `ows` is appropriate for testnet demos and bounded-budget production agents; do NOT use raw `ows` for a treasury-scale fleet without first wiring one of the delegated paths.
